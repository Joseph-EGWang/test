...
def test(model, test_loader, args):
    ...
    for batch_idx, (x, label, _) in enumerate(tqdm(test_loader)):
        ...
        if args.head=='head1':
            ...
            output = output2
        output = output[:args.num_labeled_classes]
        _, pred = output.max(1)
...
if __name__ == "__main__":
    ...
    mix_head = args.num_labeled_classes*(args.num_labeled_classes-1)/2 + args.num_labeled_classes
    # print(mix_head)


    model = ResNet(BasicBlock, [2,2,2,2], int(mix_head), args.num_unlabeled_classes).to(device)
